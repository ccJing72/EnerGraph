...
| Embedding | BAAI/bge-small-zh-v1.5（SentenceTransformers） | 中文优化，本地模型，零外部 API 依赖 |
| 向量库 | ChromaDB（本地持久化） | `data/hvac_knowledge/`，5605 条 HVAC 语料 |
...
└── src/
    ...
    │   └── java_backend.py       # 福加监控工具：10 个真实 API + Token 自动刷新（Phase 4.3）
    ├── utils/
...
| Phase 4 | Mock → 真实预测 API（TimeDiT / PhysicsAI / AIDC）+ Java 后端真实对接 | 大部分完成（Phase 4.1 能耗✅；Phase 4.2 全量抓取 17 接口 + 10 工具接入真实 API✅；Phase 4.3 Token 自动刷新✅；TimeDiT/PhysicsAI/AIDC 仍为Mock） | `docs/plan_phase4_realapi.md` + `docs/plan_phase4_realapi_batch.md` |
...
**下一步**: 可按任意顺序执行 Phase 5/6（均可并行）；前端同事可开始对接 API
